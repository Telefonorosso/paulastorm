...
#ifdef PISTORM_CLASSIC
        if (unlikely(paula_mirror_service_pending())) {
            /* Ordinary C helpers may use the 68k's reserved ARM registers.
             * Preserve the complete translated context exactly as the
             * existing miss/translation path does before calling C. */
            uintptr_t saved_armcode;
            __asm__ volatile("mov %0, x12" : "=r"(saved_armcode));
            M68K_SaveContext(ctx);
            paula_mirror_service();
            M68K_LoadContext(ctx);
            __asm__ volatile("mov x12, %0" :: "r"(saved_armcode) : "memory");
        }
#endif
