...
void secondary_boot(void)
{
    ...
    if (cpu_id == 1)
    {
        /* Always-on POC: wait for CPU0 to finish platform initialization. */
        hdmi_audio_poc_run();
        if (async_log)
            serial_writer();
    }
...
#if defined(PISTORM_ANY_MODEL) && defined(RASPI)
    /* Final boot gate: splash is ready and secondary cores are running. */
    hdmi_audio_poc_boot_test();
#endif
