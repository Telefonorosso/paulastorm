...
struct poc_text_buffer { char *p; unsigned len, cap; };
static void poc_putc(void *ctx, char c)
{
    struct poc_text_buffer *b = (struct poc_text_buffer *)ctx;
    if (b->len + 1 < b->cap) b->p[b->len++] = c;
}

/* POC3: CPU0-only diagnostic panel on the existing RGB565 splash.
 * No display reinitialization, HVS or HDMI register writes. */
                           ...
                           const char *status, unsigned seconds_left)
{
    if (!framebuffer || !pitch || !fb_width || !fb_height)
        return;
    const unsigned cols = fb_width / 8;
    const unsigned rows = fb_height / 16;
    if (cols < 12 || rows < 4)
        return;
    const unsigned panel_rows = rows < 15 ? rows : 15;
    const unsigned first = rows - panel_rows;
    const unsigned maxcols = cols < 90 ? cols : 90;
    const uint16_t bg = LE16(0x1082); /* dark neutral RGB565 */
    const uint16_t fg = LE16(0xffff);
    char footer[96];
    struct poc_text_buffer b = {footer, 0, sizeof(footer)};
    /* The formatter is Emu68's own, not libc. */
    kprintf_pc(poc_putc, &b, "Boot in %u s | Workbench will resume", seconds_left);
    footer[b.len] = 0;
    for (unsigned row = 0; row < panel_rows; ++row) {
        const char *text = "";
        if (row == 0) text = "Emu68 HDMI AUDIO POC7 LIVE - Classic";
        else if (row == panel_rows - 2) text = status ? status : "";
        else if (row == panel_rows - 1) text = footer;
        else if (row - 1 < count) text = lines[row - 1];
        unsigned y0 = (first + row) * 16;
        if (y0 + 16 > fb_height) break;
        /* Clear this row completely, including old text on refresh. */
        for (unsigned yy = 0; yy < 16; ++yy) {
            volatile uint16_t *dst = (volatile uint16_t *)((uintptr_t)framebuffer + (y0 + yy) * pitch);
            for (unsigned xx = 0; xx < fb_width; ++xx) dst[xx] = bg;
        }
        for (unsigned col = 0; col < maxcols && text[col]; ++col) {
            unsigned char c = (unsigned char)text[col];
            if (c < 32 || c > 127) c = '?';
            if (c == ' ') continue;
            uint32_t loc = (topaz8_charloc[c - 32] >> 16) >> 3;
            const uint8_t *glyph = &topaz8_chardata[loc];
            for (unsigned yy = 0; yy < 16; ++yy) {
                uint8_t bits = *glyph;
                volatile uint16_t *dst = (volatile uint16_t *)((uintptr_t)framebuffer + (y0 + yy) * pitch) + col * 8;
                for (unsigned xx = 0; xx < 8; ++xx)
                    if (bits & (0x80u >> xx)) dst[xx] = fg;
                if (yy & 1) glyph += modulo;
            }
        }
    }
}
