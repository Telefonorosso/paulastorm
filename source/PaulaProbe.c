...
static void print_snapshot(const uint32_t *v,const uint32_t *prev,int have_prev)
{
    ...
    printf("Cache underrun: zero=%lu (+%lu) recovered-pages=%lu (+%lu)\n",
           (unsigned long)v[PP_CACHE_ZERO_RETURNS],
           (unsigned long)(have_prev ? v[PP_CACHE_ZERO_RETURNS]-prev[PP_CACHE_ZERO_RETURNS] : 0u),
           (unsigned long)v[PP_CACHE_MISS_READY],
           (unsigned long)(have_prev ? v[PP_CACHE_MISS_READY]-prev[PP_CACHE_MISS_READY] : 0u));
    printf("Cache wait: frames=%lu (+%lu) recoveries=%lu (+%lu) prefetch-pages=%lu (+%lu)\n",
           (unsigned long)v[PP_CACHE_WAIT_FRAMES],
           (unsigned long)(have_prev ? v[PP_CACHE_WAIT_FRAMES]-prev[PP_CACHE_WAIT_FRAMES] : 0u),
           (unsigned long)v[PP_CACHE_WAIT_RECOVERIES],
           (unsigned long)(have_prev ? v[PP_CACHE_WAIT_RECOVERIES]-prev[PP_CACHE_WAIT_RECOVERIES] : 0u),
           (unsigned long)v[PP_CACHE_PREFETCH_PAGES],
           (unsigned long)(have_prev ? v[PP_CACHE_PREFETCH_PAGES]-prev[PP_CACHE_PREFETCH_PAGES] : 0u));
